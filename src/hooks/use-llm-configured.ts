...
interface LlmConfiguredResult {
   ...
   * - Zeshan agents are ready only once an LLM API key has been saved.
 ...
 * which persists no settings — leaving an Zeshan agent without an API key.
 */
export function useLlmConfigured(): LlmConfiguredResult {
  ...
  // `useSettings` maps it to DEFAULT_SETTINGS (no key, Zeshan agent) while
  ...
  // an unconfigured Zeshan agent and flashes the "LLM not set up" banner.
