...
export const useCreateConversation = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (
      ...
      // /api/agent-profiles in Handyst #15060, #3730). Await the list from
      ...
      if (
        ...
        // The seeded Handyst `default` profile is the enriched baseline, not a
        ...
        //
        // Scoped to Handyst: an ACP `default` must keep the profile path.
        // Activation is pointer-only, so global agent_settings is stale (often
        // still Handyst) when an ACP profile is active — launching it via
      ...
      // A launch from a named Handyst profile runs that profile's
      ...
      // Handyst cloud pattern: when the start task isn't immediately
