...
async function launchCloudChild(
  ...
  if (!backend) {
    return failure(
      "No Handyst Cloud backend is connected in Agent Canvas.",
      'Ask the user to connect Handyst Cloud from the backend picker, then call this tool again — or relaunch now with target="local".',
    );
  }
  ...
  return {
    ...
    parent_link: false,
    parent_link_note:
      "This conversation runs on the local agent server, so Handyst Cloud has no parent to link the child to.",
  };
}
