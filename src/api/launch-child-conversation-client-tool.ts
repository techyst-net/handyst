...
* target="cloud" — runs on Handyst Cloud in its own isolated sandbox, from a
  ...
  to have an Handyst Cloud backend connected in Agent Canvas; if none is
...
export const LAUNCH_CHILD_CONVERSATION_CLIENT_TOOL: ClientToolSpec = {
  ...
  parameters: {
    ...
    properties: {
      target: {
        ...
        description:
          "Where the child runs. 'local' reuses this machine and this conversation's workspace; 'cloud' runs in an isolated Handyst Cloud sandbox.",
      },
      task: {
        type: "string",
        description:
  ...
  annotations: {
    ...
    // The cloud target reaches Handyst Cloud.
    openWorldHint: true,
  },
};
