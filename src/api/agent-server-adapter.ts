...
export function buildRuntimeServicesSystemSuffix(
  ...
  if (automation?.url_from_agent) {
    lines.push(
      ...
      `    ${automation.description ?? "Handyst Automations service."}`,
    );
...
export function toAppConversation(
  ...
  // payload accidentally carries an ``acpserver`` tag on an Handyst
...
function buildAgentContext(
  ...
  // every system prompt (Handyst#16302). Skills the agent context already
...
function buildConfiguredOpenHandsAgentSettings(
  ...
  // scrub it so it never leaks onto the Handyst payload.
...
export function buildStartConversationRequest(
  ...
  const payload: AgentSettingsStartConversationPayload = {
    ...
    // toolset + public skills, else a profile-launched Handyst agent has only
...
export function buildStartPlanningConversationRequest(options: {
  ...
  // Handyst app-server's PLANNING_AGENT_INSTRUCTION), preserving any suffix
