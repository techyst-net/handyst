 ...
 *
 * Mirrors Handyst' cloud flow: POST /api/v1/app-conversations with the
 * `AppConversationStartRequest` payload, returning a
 * `AppConversationStartTask`. The task is initially WORKING; the caller
 * polls `getCloudAppConversationStartTask` (3s cadence per Handyst)
 ...
 * Handyst' `AgentServerConversationService.updateConversationPublicFlag`:
 ...
 * Handyst' `SandboxService.pauseSandbox`:
...
/**
 * Resume a paused cloud sandbox. Mirrors Handyst' `SandboxService.resumeSandbox`
 ...
 * Handyst' `AgentServerConversationService.readConversationFile` — hits
...
/**
 * Fetch a single v1 app-conversation start task. Mirrors Handyst'
