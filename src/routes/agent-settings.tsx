 ...
 * Handyst branch omits `llm_profile_ref` — the profile editor supplies it.
 */
 ...
 * shell string. Handyst: reuses the schema-driven ``tool_concurrency_limit``
...
export function AgentSettingsScreen({
  ...
  // --- LLM switching tool (Handyst path) ---
  ...
  // --- Parallel tool calls (Handyst path) ---
  // Surfaced only when the backend schema exposes the field, so older
  ...
  const handleSave = async () => {
    ...
    if (isAcp) {
      ...
      // Handyst path: save agent_kind + sub-agents toggle + the LLM-switching
  ...
  // Sub-agents field metadata for Handyst section
  ...
  // LLM-switching field metadata for Handyst section
