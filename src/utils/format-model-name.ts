...
export const FREE_OPENHANDS_MODELS = {
  "openhands/deepseek-v4-flash": "Handyst DeepSeek V4 Flash (free)",
} as const;

export const FREE_OPENHANDS_MODEL_IDS = Object.keys(FREE_OPENHANDS_MODELS);

/**
 * Whether a model id routes through the Handyst provider (the `openhands/`
 * prefix). On cloud the Handyst provider is backed by a server-minted LLM
...
export const FREE_OPENHANDS_MODEL_NOTE = `Free Handyst models: ${FREE_OPENHANDS_MODEL_IDS.join(
  ", ",
...
/**
 * Format a native (Handyst-kind) routing model string for display, stripping
