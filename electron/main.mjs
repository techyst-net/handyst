...
// Zeshan raised-hands app icon, used as the BrowserWindow.icon option.
...
function createMainWindow() {
  ...
  mainWin.webContents.setWindowOpenHandler(({ url }) => {
    // The "Login with Zeshan Cloud" device-flow opens about:blank immediately
...
async function startStack() {
  ...
  const result = await main({
    bannerTitle: "Zeshan Agent Canvas",
    staticMode: true,
    staticDir: buildDir,
    mode: "agent-canvas",
    isPublic: false,
  ...
  if (result?.agentServerReady === false) {
    throw new Error(
      "The agent server did not finish starting in time. " +
        ...
        "Python and the Zeshan agent-server from PyPI. " +
...
app.whenReady().then(async () => {
  ...
  // Zeshan logo instead of the default Electron logo. In a packaged
  ...
  if (!uvxAvailable()) {
    dialog.showErrorBox(
      ...
      app.isPackaged
        ? "The bundled uv binary could not be found. Please reinstall Zeshan Agent Canvas."
  ...
  try {
    ...
    const errorTail = recentServiceErrors.length
      ...
      : "";
    dialog.showErrorBox("Zeshan Agent Canvas failed to start", summary + errorTail);
    app.quit();
  }
});
